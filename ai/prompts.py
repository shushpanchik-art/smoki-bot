...
SAGA_SYSTEM = (
    ...
    "картриджами).\n"
    "- ОБЪЁМ: полноценный эпизод НЕ МЕНЕЕ 600 слов (цель 700-1000). "
    "Не комкай и не обрывай сюжет ради краткости — раскрывай сцены, "
    "диалоги и детали. Слишком короткий эпизод недопустим.\n"
...
SAGA_RULES = (
    ...
    "12. КАВЫЧКИ И ПРЯМАЯ РЕЧЬ: диалоги оформляй ТОЛЬКО через тире в "
    "начале реплики (— Реплика.). Ёлочки «...» — исключительно для "
    "коротких цитат внутри фразы, названий компаний/заведений и особых "
    "терминов. НИКОГДА не бери реплики диалога в ёлочки и НИКОГДА не "
    "оформляй прямую речь курсивом. Не используй \"...\" и '...'. "
    "Кавычки всегда парные: каждая « имеет свою ». Не открывай кавычку "
    "без закрытия и не закрывай без открытия.\n"
    "13. КУРСИВ <i> ТОЛЬКО для смыслового акцента на одном слове или "
    "короткой внутренней мысли героя, и то редко. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО "
    "выделять курсивом или иным тегом названия брендов, марок, моделей, "
    "компаний, гаджетов, приложений и заведений. iPhone, Porsche Panamera, "
    "VapeCorp, «Альфа» и любые подобные названия пишутся ОБЫЧНЫМ прямым "
    "текстом (компании/заведения — в ёлочках, но без курсива). Никаких "
    "<i> вокруг названий. ПРИМЕР НЕПРАВИЛЬНО: <i>«The Alchemist»</i>, "
    "<i>iPhone</i>. ПРАВИЛЬНО: «The Alchemist», iPhone — прямым "
    "текстом без тегов.\n"
    "14. СВЯЗЬ С ПРЕДЫДУЩИМ: первый абзац обязан прямо продолжать "
    "события последнего эпизода из хроники (та же сцена/линия/герой), "
    "а не начинать несвязанную историю.\n"
    "15. АТРИБУТИКА КУРЕНИЯ: главный герой (протагонист) курит "
    "ИСКЛЮЧИТЕЛЬНО вейп/электронную сигарету (пар, картриджи, под-система). Антагонисты и негативные персонажи курят ОБЫЧНЫЕ "
    "сигареты (дым, пепел, окурки). Это визуальный маркёр стороны — "
    "соблюдай его последовательно во всех сценах.\n"
)
...
SAGA_META_RULE = (
    ...
    "Сначала — текст эпизода (НЕ МЕНЕЕ 600 слов, цель 700-1000). Затем на ОТДЕЛЬНОЙ строке: "
    "===META===\n"
...
def _saga_memory(state: dict | None, summaries: list[dict] | None) -> str:
    ...
    tail = (state.get("last_tail") or "").strip()
    if tail:
        parts.append(
            "КОНЦОВКА ПРЕДЫДУЩЕГО ЭПИЗОДА (дословно; НОВЫЙ эпизод обязан "
            "начинаться как прямое продолжение ИМЕННО этой сцены — те же "
            "герои, место, момент; не начинай другую историю):\n" + tail)
