...
def facts_rules(n: int) -> str:
    ...
    n = max(1, min(3, n))
    return (
        "ЭТО УТРЕННИЙ ПОСТ (08:00-10:00). СТРОГО СОБЛЮДАЙ ФОРМАТ, "
        "он ПОЛНОСТЬЮ ОТМЕНЯЕТ любые указания об объёме в 1000 слов:\n"
        f"- Дай {n} коротких любопытных факта по теме "
        "(нумерованный список цифрами 1., 2., 3.).\n"
        "- Каждый факт — 1-2 предложения, живо и понятно.\n"
        "- В конце добавь ОДИН остроумный авторский комментарий "
        "(1-2 предложения), создающий хорошее настроение на день.\n"
...
def words_rule(words: int) -> str:
    ...
    return (
        "ЭТО ВЕЧЕРНИЙ ПОСТ (19:00-20:30). СТРОГО СОБЛЮДАЙ ФОРМАТ, "
        "он ПОЛНОСТЬЮ ОТМЕНЯЕТ любые указания об объёме в 1000 слов:\n"
