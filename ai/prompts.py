...
def facts_rules(n: int) -> str:
    """Утро: n любопытных фактов + остроумный комментарий, коротко."""
    n = max(1, min(3, n))
    return (
        f"ФОРМАТ ЭТОГО ПОСТА (переопределяет объём):\n"
        f"- Дай {n} коротких любопытных факта по теме (нумерованный список цифрами).\n"
        "- В конце добавь один остроумный авторский комментарий (1-2 предложения).\n"
        "- Тон лёгкий, утренний. Общий объём — до 120 слов.\n"
        "- НЕ пиши длинных абзацев, НЕ используй подзаголовки."
    )


def words_rule(words: int) -> str:
    """Вечер: лонг-рид на заданное число слов."""
    words = max(200, min(500, words))
    return (
        f"ФОРМАТ ЭТОГО ПОСТА (переопределяет объём):\n"
        f"- Объём: около {words} слов (не больше 500).\n"
        "- Вечерний вдумчивый лонг-рид: вступление, 1-2 блока, вывод."
    )
