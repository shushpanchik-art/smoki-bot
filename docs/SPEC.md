...
## Логика утреннего поста (новые требования, разведка 11.07)

- [ ] P2 #16 Рандом количества фактов: утренний пост генерит СЛУЧАЙНО 1-3
      факта. Сейчас `_default_length_hint` берёт фикс. morning_facts из
      settings. Нужно: при генерации утра выбирать random.randint(1, N),
      где N = morning_facts (верхняя граница, деф. 3).
- [ ] P2 #17 Минимум один свежий: в утреннем посте >=1 факт ОБЯЗАН быть из
      свежих новостей по тематике (вейпы/кальяны/табак). Усилить
      prompts.facts_rules / article_prompt для утра.
- [x] P2 #18 ИИ вдохновляется последними событиями: use_search=True уже
      включён в generate_article (Google Search grounding). Осталось усилить
      каждый промпт явным требованием актуальных новостей.

Связь с существующим бэклогом:
- P2 #7 (правила «нравится» через панель) и P2 #8 (правила цензуры через
  панель) — берём в работу первыми (PR A). FSM-ввод по образцу cb_reject.
- P2 #5 (кнопка «Назад») — заодно в PR A, если тривиально.

Ограничение Telegram (важно, скриншот черновик #29): caption у фото максимум
1024 символа. Лонг-рид (>1024) физически НЕ влезает в подпись → уходит
отдельным сообщением. Это НЕ баг. Для утра (<=120 слов) фото+текст в одном
сообщении работает; для вечернего лонг-рида — принять как есть либо
media-group.
