...
## SAGA — генеративный сюжетный сериал (канал)

Автономный сериал, который бот сам пишет и публикует эпизодами прямо в канал
(без модерации), reply-цепочкой. Каждый эпизод — 600-1000 слов, построен по
Кругу Хармона (стадии `harmon_stage`) и пирамиде Фрейтага.

### Состояние: таблица `saga_state` (одна строка, id=1)

| поле | смысл |
| --- | --- |
| `arc_number` | номер текущей арки |
| `episode_number` | сквозной номер эпизода (растёт всегда) |
| `episode_in_arc` | номер эпизода внутри арки (в хедере: «эпизод N») |
| `harmon_stage` | текущая стадия Круга Хармона |
| `arc_goal` / `arc_plan_json` / `planned_arc_length` | цель, план и длина арки |
| `pending_arc_seed` | задел следующей арки (от модели заранее) |
| `arc_synopsis_json` | синопсисы завершённых арк |
| `last_summary` | краткое содержание последнего эпизода |
| `last_tail` | последние ~800 символов тела эпизода (для дословной связности) |
| `prev_message_id` | id последнего сообщения (для reply-цепочки) |
| `prev_first_message_id` | id первого сообщения (для ссылки-стрелки с прошлого эпизода) |

Ещё две таблицы: `saga_summaries` (хроника всех эпизодов) и `saga_posts`
(журнал message_id частей — контроль живучести постов в канале).

### Связность эпизодов

- `last_tail` + инструкция промпта «КОНЦОВКА ПРЕДЫДУЩЕГО ЭПИЗОДА (дословно;
  НОВЫЙ эпизод обязан продолжать ту же сцену/линию/героя)» — новый эпизод не
...
1. `resolve_live_tail` — сверка живучести прошлых постов, откат state если
   удалены из канала.
2. `generate_episode()` → тело + META; `_apply_meta` двигает `saga_state`.
3. Хедер: `круг {arc}, эпизод {ep}` — **номер эпизода в заголовке, без эмодзи**.
4. Эпизод шлётся частями reply-цепочкой; на последней части — кнопка
   «Продолжение следует…».
5. Кнопка предыдущего эпизода правится на **ссылку-стрелку** на новый эпизод
   (`edit_message_reply_markup`) → навигация prev↔next.
6. `set_prev_ids(last, first)` и `add_saga_posts` фиксируют id.

### dry_run (тестовый прогон)

`generate_episode(dry_run=True)` / `_publish_saga_episode(dry_run=True)`:
НИЧЕГО не публикуется и не пишется в БД, `resolve_live_tail` не вызывается,
`_apply_meta` пропускается — **счётчики эпизодов НЕ растут**. Тестовые прогоны
не засоряют хронику и не двигают сагу.

### Правила оформления текста (в промпте, ai/prompts.py)

- **Прямая речь — курсивом** `<i>…</i>`; НЕ использовать `"..."` и `'...'`.
- Кавычки только парные ёлочки «…»: каждая « имеет свою ».
- **Бренды/марки/модели/заведения — БЕЗ курсива.** Компании/заведения — в
  ёлочках, но без `<i>`. Пример НЕПРАВИЛЬНО: `<i>«The Alchemist»</i>`.
- Курение как визуальный маркёр стороны: протагонисты курят
  ИСКЛЮЧИТЕЛЬНО вейп/электронную сигарету; антагонисты — обычные сигареты.

### Исправленные баги (прод, скриншот группы)

1. Ссылка на предыдущий эпизод — кнопка-стрелка prev→next.
2. Текст нового эпизода дословно связан с предыдущим (`last_tail`).
3. Номер эпизода в заголовке («эпизод N»).
4. В прошлом сообщении появляется ссылка-стрелка на новый эпизод.
5. Эмодзи перед названием саги убраны из хедера.
6. Кавычки/курсив: прямая речь курсивом, парные ёлочки.
7. Названия брендов (iPhone, Porsche Panamera и т.п.) — без курсива.
