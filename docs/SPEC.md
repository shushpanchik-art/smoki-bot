...
- [ ] U9a (P2) РАСХОД В $ + ПРОГНОЗ ПО ЗАДАННОМУ БЮДЖЕТУ (реализуемо дёшево):
...
- [ ] U9b (P3, крупный этап) РЕАЛЬНЫЙ БАЛАНС Google Cloud через Cloud Billing
...
## U10 — Onboarding: подписчик бота → приглашение в канал @SMOKTOLK [БЭКЛОГ]
...
- [ ] U10a (P2) Разведка smoktolk_bot: где база users, есть ли /start
      onboarding, привязан ли канал @SMOKTOLK, права бота в канале.
- [ ] U10b (P2) В smoktolk_bot при первом /start: сохранить user в БД +
      отправить приветствие с inline-кнопкой «Подписаться на @SMOKTOLK»
      (invite-ссылка или t.me/SMOKTOLK если канал публичный). Идемпотентно
      (повторный /start не спамит). Опционально: отметка joined после
      проверки членства (getChatMember) при следующем контакте.
- [ ] U10c (P3) Метрика: сколько нажали /start и сколько реально подписались.
