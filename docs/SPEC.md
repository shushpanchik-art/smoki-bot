...
- [ ] U1 (P2) Токены/квота в статистике (= P3 #4 с UX-акцентом): суммировать
  `usage_metadata.total_token_count` в ai_logs, показывать «Токенов
  израсходовано» в панели. Админ видит расход, чтобы не упереться в лимит
  Vertex внезапно. Реализуемо без Cloud Quotas API.
- [ ] U2 (P2) Предпросмотр перед автопубликацией. При простое пост уходит по
  дедлайну без глаз человека. За X минут до дедлайна — сообщение админу
  «через 15 мин уйдёт автоматически» с кнопками [Стоп]/[Опубликовать сейчас].
...
- [ ] S1 (P3) gitleaks в pre-commit локально (не только CI/публикация) — после
